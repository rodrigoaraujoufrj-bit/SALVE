...
if os.path.exists(FOTOS):
    ...
    for i, nome in enumerate(df.nome_cientifico):
        ...
        if r.foto_id and r.licenca in LICENCAS_ACEITAS:
            e = [int(r.foto_id), int(r.taxon_id) if r.taxon_id else 0,
                 "s" if r.host == "s3" else "t", r.licenca, r.autor]
            ext = getattr(r, "ext", "") if "ext" in f.columns else ""
            if ext:
                e.append(ext)          # extensao conhecida: uma tentativa so
            fotos[i] = e
            n_img += 1
        elif r.taxon_id:
...
// O iNaturalist preserva a extensao do arquivo original. Quando a coleta nao a
// registrou, tenta nesta ordem antes de desistir e cair para o link.
var EXTENSOES = ["jpeg", "jpg", "png"];
...
function mostrarFoto(o){
  ...
  var base = (SERVIDOR[f[2]] || SERVIDOR.t) + f[0] + "/medium.";
  var lista = f[5] ? [f[5]] : EXTENSOES;    // extensao conhecida: uma tentativa so
  img.onload = function(){ fig.hidden = false; };
  (function tenta(i){
    if (i >= lista.length){                 // acabaram as extensoes: cai para o link
      fig.hidden = true;
      if (f[1]){ alt.href = "https://www.inaturalist.org/taxa/" + f[1]; alt.hidden = false; }
      return;
    }
    img.onerror = function(){ tenta(i + 1); };
    img.src = base + lista[i];
  })(0);
}
